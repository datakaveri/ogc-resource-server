...
# **get_features**
> FeatureCollectionGeoJSON get_features(bbox_crs=bbox_crs, crs=crs, bbox=bbox, datetime=datetime, limit=limit, offset=offset, name_of_to=name_of_to, district=district, elevation=elevation, scale=scale, state=state, rotation=rotation)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Get features from Fetched from Administrative_Boundary_Database_For_Entire_Country_Upto_Taluk_level_with_HQ
        api_response = api_instance.get_features(bbox_crs=bbox_crs, crs=crs, bbox=bbox, datetime=datetime, limit=limit, offset=offset, name_of_to=name_of_to, district=district, elevation=elevation, scale=scale, state=state, rotation=rotation)
