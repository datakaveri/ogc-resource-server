# openapi_client.NiruthisGranularWeatherInsightsForTelanganaApi
...
# **get_features**
> FeatureCollectionGeoJSON get_features(bbox_crs=bbox_crs, crs=crs, bbox=bbox, datetime=datetime, limit=limit, offset=offset, avgwind=avgwind, tmax=tmax, tmin=tmin, gp=gp, rhmin=rhmin, rhavg=rhavg, observation_date_time=observation_date_time, maxwind=maxwind, daylength=daylength, tdew=tdew, subdistrict=subdistrict, srad=srad, district=district, rhmax=rhmax, prcp=prcp, village=village)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Get features from Daily (7 days prior current day) information about weather parameters for villages of Telangana
        api_response = api_instance.get_features(bbox_crs=bbox_crs, crs=crs, bbox=bbox, datetime=datetime, limit=limit, offset=offset, avgwind=avgwind, tmax=tmax, tmin=tmin, gp=gp, rhmin=rhmin, rhavg=rhavg, observation_date_time=observation_date_time, maxwind=maxwind, daylength=daylength, tdew=tdew, subdistrict=subdistrict, srad=srad, district=district, rhmax=rhmax, prcp=prcp, village=village)
